...
async function deploy() {
  ...
  const contract = await factory.deploy("xyz", "my token");
...
describe("LazyFactory", function () {
  it("Should deploy", async function () {
    ...
    const contract = await LazyFactory.deploy("xyz", "my token");
  ...
  it("Should redeem an NFT from a signed voucher", async function () {
    ...
    const theSellingPrice = ethers.utils.parseUnits("0.0008", "ether");
